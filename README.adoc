...
- **Cloud Storage**: Integration with cloud document repositories
...
- **WebSocket Support**: Real-time search and notifications
...
./mvnw clean install -pl turing-js-sdk       # JavaScript SDK
----
...
java -jar turing-app/target/turing-app-{viglet-version}.jar --spring.profiles.active=production
----
...
A: Ensure you're using Java 17 or higher. Check with `java -version`.
