...
class TurJobItemWithSessionTest {
    ...
    void testRecordCreationWithAllFields() {
        ...
        TurJobItemWithSession jobItemWithSession = new TurJobItemWithSession(
                jobItem, session, dependencies, standalone
        );
    ...
    void testRecordEquality() {
        ...
        TurJobItemWithSession item1 = new TurJobItemWithSession(
                jobItem1, session, dependencies, true
        );
        TurJobItemWithSession item2 = new TurJobItemWithSession(
                jobItem1, session, dependencies, true
        );
        TurJobItemWithSession item3 = new TurJobItemWithSession(
                jobItem2, session, dependencies, true
        );
    ...
    void testRecordToString() {
        ...
        TurJobItemWithSession jobItemWithSession = new TurJobItemWithSession(
                jobItem, session, dependencies, true
        );
    ...
    void testRecordImmutability() {
        ...
        Set<String> originalDependencies = new HashSet<>(Arrays.asList("dep1", "dep2"));
        Set<String> dependencies = new HashSet<>(originalDependencies);

        TurJobItemWithSession jobItemWithSession = new TurJobItemWithSession(
                jobItem, session, dependencies, false
        );

        // Modify the original set
        dependencies.add("dep3");

        // The record should still have the original dependencies
        assertThat(jobItemWithSession.dependencies()).containsExactlyInAnyOrderElementsOf(originalDependencies);
        assertThat(jobItemWithSession.dependencies()).doesNotContain("dep3");
    }
}