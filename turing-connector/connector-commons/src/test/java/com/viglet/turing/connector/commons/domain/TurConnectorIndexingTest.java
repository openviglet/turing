...
class TurConnectorIndexingTest {
    ...
    void testBuilderWithAllFields() {
        ...
        TurConnectorIndexing indexing = TurConnectorIndexing.builder().id(id).objectId(objectId)
                .source(source).environment(environment).transactionId(transactionId)
                .checksum(checksum).locale(locale).created(created)
                .modificationDate(modificationDate).status(status).sites(sites).build();
    ...
    void testBuilderWithMinimalFields() {
        ...
        TurConnectorIndexing indexing =
                TurConnectorIndexing.builder().objectId(objectId).source(source).build();
    ...
    void testBuilderWithNullValues() {
        TurConnectorIndexing indexing = TurConnectorIndexing.builder().objectId(null).source(null)
                .environment(null).transactionId(null).checksum(null).locale(null).created(null)
                .modificationDate(null).status(null).sites(null).build();
    ...
    void testBuilderChaining() {
        TurConnectorIndexing indexing =
                TurConnectorIndexing.builder().id(789).objectId("chained-object-id")
                        .source("chained-source").environment("chained-environment").build();
