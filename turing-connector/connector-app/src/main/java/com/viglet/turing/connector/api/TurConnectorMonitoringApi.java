...
public class TurConnectorMonitoringApi {
        ...
        @GetMapping("{source}")
        public ResponseEntity<TurConnectorMonitoring> monitoringIndexingBySource(
                        @PathVariable String source) {
                List<TurConnectorIndexingModel> indexing = indexingService
                                .getBySourceAndProvider(source, plugin.getProviderName());
                return generateMonitoringResponse(indexing);
        }

        @PostMapping
        public ResponseEntity<TurConnectorMonitoring> indexContentId(
                        @RequestBody List<String> contentIds) {
                List<TurConnectorIndexingModel> indexing =
                                indexingService.findAllByProviderAndObjectIdIn(
                                                plugin.getProviderName(), contentIds);
                return generateMonitoringResponse(indexing);
        }

        private ResponseEntity<TurConnectorMonitoring> generateMonitoringResponse(
                        List<TurConnectorIndexingModel> indexing) {
                return ResponseEntity.ok(indexing.isEmpty() ? new TurConnectorMonitoring()
