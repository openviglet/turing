...
public class TurFileUtils {
    ...
    private static TurTikaFileAttributes getTurTikaFileAttributes(File file,
    ...
    public static TurFileAttributes documentToText(MultipartFile multipartFile) {
        return Optional.ofNullable(parseFile(multipartFile))
                .map(tikaFileAttributes -> getTurFileAttributes(parseFile(multipartFile),
    ...
    public static TurFileAttributes urlContentToText(URL url) {
        ...
        return Optional.ofNullable(getFile(url)).map(f -> {
            ...
            return Optional.ofNullable(parseFile(f))
                    .map(tikaFileAttributes -> getTurFileAttributes(tikaFileAttributes,
    ...
    private static TurFileAttributes getTurFileAttributes(TurTikaFileAttributes tikaFileAttributes,
        ...
        return Optional.ofNullable(tikaFileAttributes)
                .map(attributes -> TurFileAttributes.builder().content(attributes.getContent())
                        .name(fileName).extension(fileExtension).size(new TurFileSize(fileSize))
