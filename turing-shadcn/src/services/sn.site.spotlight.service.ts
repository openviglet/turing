...
export class TurSNSiteSpotlightService {
  ...
  async query(siteId: string): Promise<TurSNSiteSpotlight[]> {
    const response = await axios.get<TurSNSiteSpotlight[]>(`/sn/${siteId}/spotlight`);
  ...
  async create(turSNSiteSpotlight: TurSNSiteSpotlight): Promise<TurSNSiteSpotlight> {
  ...
  async update(turSNSiteSpotlight: TurSNSiteSpotlight): Promise<TurSNSiteSpotlight> {
