...
export const LoggingInstanceForm: React.FC<Props> = ({ value, isNew }) => {
  const form = useForm<TurLoggingInstance>();
  const { setValue } = form;
  ...
  useEffect(() => {
    ...
    setValue("turLoggingVendor", value.turLoggingVendor);
    setValue("url", value.url);
  }, [setValue, value]);


  function onSubmit(seInstance: TurLoggingInstance) {
    try {
      if (isNew) {
