...
interface Props {
  items: TurLoggingInstance[] | undefined;
}

export const LoggingCardList: React.FC<PropsWithChildren<Props>>= ({ items }) => {
            ...
            <Card key={item.id} className="@container/card">
                ...
                <CardFooter className="flex-col gap-2">
                    <Button type="submit" className="w-full" asChild>
                        <NavLink to={`${ROUTES.LOGGING_INSTANCE}/${item.id}`}>Edit</NavLink>
                    </Button>
                </CardFooter>
            </Card>
        ))}
