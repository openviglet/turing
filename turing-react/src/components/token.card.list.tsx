...
interface Props {
  items: TurTokenInstance[] | undefined;
}

export const TokenCardList: React.FC<PropsWithChildren<Props>>= ({ items }) => {
            ...
            <Card key={item.id} className="@container/card">
                ...
                <CardFooter className="flex-col gap-2">
                    <Button type="submit" className="w-full" asChild>
                        <NavLink to={"/console/token/instance/" + item.id}>Edit</NavLink>
                    </Button>
                </CardFooter>
            </Card>
        ))}
